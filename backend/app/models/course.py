...
class UserCourse(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "ca_score": float(self.ca_score) if self.ca_score else 0.0,
            "participation_score": float(self.participation_score) if self.participation_score else None,
            "exam_score": float(self.exam_score) if self.exam_score else None,
            "predicted_exam_score": float(self.predicted_exam_score) if self.predicted_exam_score else None,
            "current_score": float(self.current_score) if self.current_score else None,
            "predicted_score": float(self.predicted_score) if self.predicted_score else None,
            "current_grade_point": float(self.current_grade_point) if self.current_grade_point else None,
            "predicted_grade_point": float(self.predicted_grade_point) if self.predicted_grade_point else None,
